...
const DashboardPage = () => {
  ...
  const handleDeleteVideo = async (e, videoId, videoTitle) => {
    ...
    try {
      ...
      const totalAnnotations = updatedVideos.reduce((sum, video) => {
        return sum + (video.annotationCount || 0);
      }, 0);
      
      const totalBookmarks = updatedVideos.reduce((sum, video) => {
        return sum + (video.bookmarkCount || 0);
      }, 0);
            ...
            {videos.map((video) => (
              <div key={video.id} className="relative group">
                <Link to={`/video/${video.id}`}>
                  <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-all hover:-translate-y-1 cursor-pointer">
